...
namespace PolyAndCode.UI
{
    ...
    public class HorizontalRecyclingSystem : RecyclingSystem
    {
        ...
        public override void Init(Action onInitialized)
        {
            ...
            //Set content width according to no of colomn
            int colomns = Mathf.CeilToInt(((float)_itemCount) / _rows);

            float contentXSize = colomns * _cellWidth + (colomns - 1) * Spacing.x + Padding.left + Padding.right;
        ...
        public override Vector2 ScrollToItem(int index)
        {
            ...
            if (IsReverse)
            {
                float contentXPos = -(coloums - 1) * (_cellWidth + Spacing.x) - Padding.right + Viewport.rect.size.x / 2 - _cellWidth / 2;
                return Vector2.left * Mathf.Clamp(contentXPos,Viewport.rect.size.x - Content.rect.size.x, 0f);
            }
            else
            {
        ...
        private void ResetCellPool()
        {
            ...
            while ((poolSize < minPoolSize || currentPoolCoverage < requriedCoverage) && poolItem < _itemCount)
            {
                ...
                else
                {
                    posY = (Padding.bottom - Padding.top) / 2;
                    item.anchoredPosition = new Vector2(posX, posY);
                    posX = IsReverse ? item.anchoredPosition.x - _cellWidth - Spacing.x : item.anchoredPosition.x + _cellWidth + Spacing.x;
                    currentPoolCoverage += _cellWidth + Spacing.x;
                }
